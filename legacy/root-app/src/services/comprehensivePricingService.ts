...
// Service Fee Commission Rates
export const SERVICE_COMMISSIONS = {
  VERIFIED_DRIVER_BONUS: 2, // +2% commission
  INSURANCE_HANDLING: 5, // +5% commission
  PACKING_MARKUP: 10, // +10% commission
  CLEANING_COORDINATION: 10, // +10% commission
  EXPRESS_DELIVERY: 4 // +4% commission (split with driver)
};
...
export function calculateFare(
  ...
  // Add service fee commissions
  if (addOns.insurance) commissionRate += SERVICE_COMMISSIONS.INSURANCE_HANDLING;
  if (addOns.packing) commissionRate += SERVICE_COMMISSIONS.PACKING_MARKUP;
  if (addOns.cleaning) commissionRate += SERVICE_COMMISSIONS.CLEANING_COORDINATION;
  if (addOns.express) commissionRate += SERVICE_COMMISSIONS.EXPRESS_DELIVERY;
...
/**
 * Get available vehicle classes for a given load description (AI-powered recommendation)
 */
export function getRecommendedVehicleClasses(loadDescription: string, distance: number): VehicleClass[] {
  // This would integrate with ReloAI for intelligent recommendations
  // For now, return all classes with simple logic
  ...
  if (distance > 1000) {
    // Long distance: exclude motorbikes
    return VEHICLE_CLASSES.filter(vc => vc.id !== 'motorbike');
  }
...
/**
 * Calculate surge pricing based on demand factors
 */
export function applySurgePricing(baseFare: number, demandFactor: number): number {
  // Surge pricing: 1.0 = normal, 1.5 = 50% increase, 2.0 = 100% increase
  const surgeFactor = Math.max(1.0, Math.min(3.0, demandFactor)); // Cap at 3x
  return baseFare * surgeFactor;
}

/**
 * Calculate estimated time and fuel cost for route optimization
 */
...
export function calculateRouteEstimate(distance: number, vehicleClassId: string): RouteEstimate {
  ...
  // Estimated parameters (would be refined with real data)
  const avgSpeedKmh = distance > 100 ? 80 : 50; // Highway vs city
  const estimatedDuration = (distance / avgSpeedKmh) * 60; // Convert to minutes

  // Fuel consumption estimates (L/100km)
  const fuelConsumption = getFuelConsumption(vehicleClassId);
  const fuelPrice = 22.50; // ZAR per liter (average)
  const estimatedFuelCost = (distance / 100) * fuelConsumption * fuelPrice;

  // Toll fees (estimated based on distance and major routes)
...
function calculateTollFees(distance: number): number {
  // Simplified toll calculation (would use actual toll gate data)
  if (distance < 50) return 0;
  if (distance < 200) return 25;
  if (distance < 500) return 85;
  if (distance < 1000) return 175;
  return 285; // Long distance N2/N3 routes
}
